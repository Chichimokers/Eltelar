...
export class TelaController {
  ...
  constructor(private readonly telasService: TelaService) {}
  ...
  async create(@UploadedFile() file: Express.Multer.File, @Body() tela: Tela): Promise<Tela> {
  ...
  @Get()
  findAll(): Promise<Tela[]> {
    return this.telasService.findAll();
  }

  @Get(':id')
  findOne(@Param('id') id: number): Promise<Tela> {
    return this.telasService.findOne(id);
  }

  @Put(':id')
  update(@Param('id') id: number, @Body() tela: Partial<Tela>): Promise<Tela> {
